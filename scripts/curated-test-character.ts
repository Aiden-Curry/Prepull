...
const fixtures = { "fresh-60": eraFuryFixtures.fresh60, "pre-raid": eraFuryFixtures.preRaid, "mc-progression": eraFuryFixtures.moltenCore, "near-bis": eraFuryFixtures.nearBis };
...
if (requested && !fixtures[requested as keyof typeof fixtures]) throw new Error(`Unknown fixture ${requested}. Use fresh-60, pre-raid, mc-progression, near-bis, or --all.`);
...
console.log(`\nCanonical set: ${getCanonicalCuratedProfile().sets.find((set) => set.id === "era-fury-pre-raid")?.name}; status draft; publishable NO.`);
