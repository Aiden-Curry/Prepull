...
import { availabilityFor, isAvailableInPhase, resolvedPhaseForSet, type ClassicContentPhase } from "../lib/curated-gear/availability.ts";
...
for (const group of new Map(Object.entries(set.slots)).values()) { if (!group?.length) continue; console.log(`\n${group[0].slot.toUpperCase()}`); for (const entry of group) { const item = metadata.get(entry.itemId); const eligible = isAvailableInPhase(entry.itemId, availabilityPhase); if (eligible) eligibleRows += 1; else unavailableRows += 1; console.log(`${item?.name ?? `UNRESOLVED ITEM #${entry.itemId}`} (#${entry.itemId})${eligible ? "" : `\n   Status: unavailable\n   Available from: Phase ${availabilityFor(entry.itemId)?.availableFromPhase ?? "unknown"}`}\n   Tier: ${entry.tier}\n   Source: ${entry.source.instance ?? entry.source.zone ?? entry.source.quest ?? entry.source.type}${entry.source.boss ? ` — ${entry.source.boss}` : ""}\n   Verification: ${entry.provenance.verificationStatus}`); } }
