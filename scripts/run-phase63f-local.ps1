...
try {
  ...
  if (-not $ready) { Receive-Job $server; throw "Local E2E server did not become ready." }
  ...
  npx playwright test `
    ...
    tests/e2e/recommendation-coverage.e2e.spec.ts `
    --project=chromium `
    --workers=1
  if ($LASTEXITCODE -ne 0) { throw "Playwright exited with $LASTEXITCODE." }
} finally {
