...
const itemFor = (reference: ReferenceItemEntry, character?: NormalizedCharacter, phase: ClassicContentPhase = 6): EvaluatedItem | undefined => {
  ...
  return { ...source, slot: reference.slot, source: factionSource ? { ...reference.source, quest: factionSource } : phaseSource ?? reference.source, uniqueGroup: reference.uniqueGroup ?? source.uniqueGroup, reference, isRealistic: "isRealistic" in source ? source.isRealistic : undefined };
};
...
export function evaluateCuratedReference(character: NormalizedCharacter, profile: CuratedReferenceProfile = getCanonicalCuratedProfile(), setId = profile.defaultSetId, requestedAvailabilityPhase?: ClassicContentPhase): CuratedEvaluation {
  ...
  const activityView = (groups: Record<string, CuratedRecommendation[]>, view: CuratedActivity["view"]): CuratedActivity[] => Object.entries(groups).map(([activity, upgrades]) => { const score = upgrades.reduce((total, recommendation) => total + priorityScore(recommendation), 0); const tierItems = upgrades.flatMap((upgrade) => optionItems(upgrade)); const highestTier = tierItems.map((item) => item.reference.tier).sort((left, right) => tierRank[right] - tierRank[left])[0] ?? "entry"; const raid = view === "raid-progression"; const singular = raid ? "raid progression opportunity" : "realistic improvement"; const plural = raid ? "raid progression opportunities" : "realistic improvements"; return { activity, upgrades, slots: [...new Set(upgrades.map((upgrade) => upgrade.slot))], highestTier, score, view, lockoutType: raid ? "weekly" as const : undefined, recommendation: `${upgrades.length} curated ${upgrades.length === 1 ? singular : plural} across ${[...new Set(upgrades.map((upgrade) => upgrade.slot))].join(", ")}.` }; }).sort((left, right) => right.score - left.score);
  const realisticGroups = referenceAvailability === "available" ? recommendations.filter((recommendation) => recommendation.bestRealistic && ["major-opportunity", "meaningful-upgrade", "upgrade"].includes(recommendation.priority)).reduce<Record<string, CuratedRecommendation[]>>((result, recommendation) => { const activity = sourceName(recommendation); (result[activity] ??= []).push(recommendation); return result; }, {}) : {};
  const raidGroups = referenceAvailability === "available" ? recommendations.filter((recommendation) => optionItems(recommendation).some((item) => item.reference.raidOrigin || item.reference.requiresRaidContent || item.source?.type === "Raid")).reduce<Record<string, CuratedRecommendation[]>>((result, recommendation) => { const raidOption = optionItems(recommendation).find((item) => item.reference.raidOrigin || item.reference.requiresRaidContent || item.source?.type === "Raid"); const activity = raidOption ? sourceName({ target: raidOption }) : "Unknown raid"; (result[activity] ??= []).push(recommendation); return result; }, {}) : {};
  const realisticActivities = activityView(realisticGroups, "realistic-non-raid");
  const raidActivities = activityView(raidGroups, "raid-progression");
  return { profileId: profile.id, strategy: profile.strategy, methodology: profile.methodology, set: activeSet, recommendations, activities: realisticActivities, realisticActivities, raidActivities, incompleteData: !activeSet.complete, referenceAvailability, characterProgressionPhase: characterPhase, requestedSetId: requestedSet?.id ?? baseSet.id };
}
