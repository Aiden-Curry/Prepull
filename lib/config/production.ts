...
export function validateProductionConfig(env: NodeJS.ProcessEnv = process.env): ProductionConfigIssue[] {
  ...
  if (!env.BATTLE_NET_STATE_SECRET || env.BATTLE_NET_STATE_SECRET.length < 32) issues.push({ field: "BATTLE_NET_STATE_SECRET", message: "must be an independent secret of at least 32 characters" });
  if (!env.BATTLENET_CLIENT_ID) issues.push({ field: "BATTLENET_CLIENT_ID", message: "is required for Battle.net authorization" });
  if (!env.BATTLENET_CLIENT_SECRET) issues.push({ field: "BATTLENET_CLIENT_SECRET", message: "is required for the server-side authorization-code exchange" });
  if (env.BATTLENET_OAUTH_MODE === "mock") issues.push({ field: "BATTLENET_OAUTH_MODE", message: "mock OAuth is forbidden in production" });
