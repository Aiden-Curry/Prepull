...
export async function loadPrimaryAdvice() {
  ...
  if (latestSync) {
    ...
    const advice = buildPlayerAdvice(character).advice; const previousCharacter = previousSync ? characterFromSync({ id: primary.id, name: primary.characterName, region: primary.region, realmName: primary.realmName, contentVersion: primary.contentVersion, characterRealmType: primary.characterRealmType }, previousSync) : undefined; const previousAdvice = previousCharacter ? buildPlayerAdvice(previousCharacter).advice : undefined; const targetIds = new Set(previousAdvice ? [...previousAdvice.topActions, ...previousAdvice.secondaryActions].flatMap((action) => action.targets.map((target) => target.itemId)) : []); const progress = diffEquipment(previousSync, latestSync); progress.completedTargets = completedTargetNames(previousCharacter, character, targetIds);
