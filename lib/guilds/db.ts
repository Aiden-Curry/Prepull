...
export async function withTransaction<T>(run: (client: PoolClient) => Promise<T>) { let client: PoolClient | undefined; try { client = await getPool().connect(); await client.query("BEGIN"); const result = await run(client); await client.query("COMMIT"); return result; } catch (error) { if (client) { try { await client.query("ROLLBACK"); } catch { /* preserve the normalized operation error */ } } if (error instanceof Error && error.name.endsWith("DomainError")) throw error; throw mapPostgresError(error); } finally { client?.release(); } }
