...
export async function requireAuthenticatedUser() { const { getServerSession } = await import("next-auth"); const session = await getServerSession(authOptions); if (!session?.user?.id) redirect("/auth/signin"); return { id: session.user.id, displayName: session.user.name ?? session.user.email ?? "PrePull user" }; }
