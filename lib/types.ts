...
export type ItemQuality = "Unknown" | "Poor" | "Common" | "Uncommon" | "Rare" | "Epic" | "Legendary";
export type EquipmentSlot = "Shirt" | "Tabard" | "Head" | "Neck" | "Shoulder" | "Back" | "Chest" | "Wrist" | "Hands" | "Waist" | "Legs" | "Feet" | "Finger 1" | "Finger 2" | "Trinket 1" | "Trinket 2" | "Main Hand" | "Off Hand / Shield" | "Ranged / Relic";
...
export type EquippedItem = { itemId: number; name: string; slot: EquipmentSlot; quality: ItemQuality; itemLevel?: number; icon: string; enchantIds?: number[]; gemIds?: number[]; stats: GearStats; enchantments?: string[]; weapon?: WeaponProperties; setId?: string; specialEffectId?: string; uniqueGroup?: string; source?: CharacterSource; missing?: boolean };
export type CharacterTalent = { name: string; rank: number };
export type ProviderName = "mock" | "blizzard";
export type CharacterDataMeta = { provider: ProviderName; retrievedAt?: string; lastUpdated?: string; isLive: boolean };
export type NormalizedCharacter = { id: string; name: string; region: Region; realm: string; contentVersion: ContentVersion; realmType: CharacterRealmType; level: number; race: string; class: string; spec: string; faction: Faction; professions: string[]; equipmentStatus?: "available" | "unavailable"; armory?: ArmorySnapshot; talents?: CharacterTalent[]; weaponSkillContext?: WeaponSkillContext; equipment: EquippedItem[]; curatedReferenceSetId?: string; dataMeta?: CharacterDataMeta };
