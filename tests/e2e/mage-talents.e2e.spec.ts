...
for (const width of [1440, 390]) test(`Frost build switching, trees and details at ${width}px`, async ({ page }) => {
  await page.setViewportSize({ width, height: 900 });
  await page.goto("/era/guides/classes/mage/frost");
  const tree = page.locator("[data-guide-talent-build]");
  for (const [id, allocation, talent, rank] of [
    ["era-frost-arcane-power", "31/0/20", "arcanePower", "1"],
    ["era-frost-winters-chill", "16/0/35", "wintersChill", "5"],
    ["era-frost-arcane-power", "31/0/20", "arcanePower", "1"],
  ]) {
    ...
    for (const name of ["arcane", "fire", "frost"]) {
    ...
    } else await page.getByLabel("Talent tree", { exact: true }).selectOption(talent === "arcanePower" ? "arcane" : "frost");
