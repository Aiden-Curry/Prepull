...
for (const width of [1440, 390]) test(`Hunter build switching, trees and details at ${width}px`, async ({ page }) => {
  await page.context().route("https://wow.zamimg.com/js/tooltips.js", route => route.abort());
  await page.setViewportSize({ width, height: 900 });
  await page.goto("/era/guides/classes/hunter/marksmanship");
  const tree = page.locator("[data-guide-talent-build]");
  for (const [id, allocation, talent, rank] of [
    ["era-marksmanship-standard", "20/31/0", "trueshotAura", "1"],
    ["era-marksmanship-surefooted", "2/31/18", "surefooted", "3"],
    ["era-marksmanship-standard", "20/31/0", "trueshotAura", "1"],
  ]) {
    ...
    for (const name of ["beast mastery", "marksmanship", "survival"]) {
    ...
    } else await page.getByLabel("Talent tree", { exact: true }).selectOption(talent === "trueshotAura" ? "marksmanship" : "survival");
