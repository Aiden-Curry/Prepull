...
test("BWL Logs links follow encounter IDs despite misleading display names", async ({ page }) => {
  ...
  try {
    ...
    await expect(logs.getByRole("link", { name: "Chromaggus guide", exact: true })).toHaveAttribute("href", `${path}/chromaggus`);
    await expect(logs.getByRole("link", { name: "Nefarian guide", exact: true })).toHaveAttribute("href", `${path}/nefarian`);
    await expect(logs.locator(`a[href^="${path}/"]`)).toHaveCount(8);
  } finally {
...
test("BWL discovery, eight registry cards and no unpublished boss links", async ({ page, request }) => {
  ...
  expect(await cards.getByRole("link").evaluateAll(nodes => nodes.map(node => Number(node.getAttribute("data-encounter-id"))))).toEqual([50610, 50611, 50612, 50613, 50614, 50615, 50616, 50617]);
  await expect(cards.getByText("Boss guide not yet published.", { exact: true })).toHaveCount(0);
  ...
  for (const boss of raidProgressionEncounters("era", 2002)) {
    ...
    expect((await request.get(`${path}/${slug}`)).status()).toBe(200);
  }
});
...
test("BWL SEO, breadcrumbs, sections and TBC separation", async ({ page, request }) => {
  ...
  expect(xml).toContain(`${path}/chromaggus`);
  expect(xml).toContain(`${path}/nefarian`);
...
for (const [slug, title, sibling, direction] of [
  ...
  ["chromaggus", "Chromaggus", "nefarian", "Next"],
  ["nefarian", "Nefarian", "chromaggus", "Previous"],
  ...
  await expect(navigation.getByRole("link")).toHaveCount(["vaelastrasz-the-corrupt", "broodlord-lashlayer", "firemaw", "ebonroc", "flamegor", "chromaggus"].includes(slug) ? 2 : 1);
