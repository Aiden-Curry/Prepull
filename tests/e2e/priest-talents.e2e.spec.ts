...
for (const width of [1440, 390]) test(`Priest build switching, trees and details at ${width}px`, async ({ page }) => {
  await page.context().route("https://wow.zamimg.com/js/tooltips.js", route => route.abort());
  await page.setViewportSize({ width, height: 900 });
  await page.goto("/era/guides/classes/priest/holy");
  const tree = page.locator("[data-guide-talent-build]");
  for (const [id, allocation, talent, rank] of [
    ["era-holy-standard", "21/30/0", "spiritualHealing", "5"],
    ["era-holy-power-infusion", "32/19/0", "powerInfusion", "1"],
    ["era-holy-standard", "21/30/0", "spiritualHealing", "5"],
  ]) {
    ...
    for (const name of ["discipline", "holy", "shadow"]) {
    ...
    } else await page.getByLabel("Talent tree", { exact: true }).selectOption(talent === "spiritualHealing" ? "holy" : "discipline");
