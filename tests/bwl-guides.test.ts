...
test("BWL overview maps to the accepted eight encounters and excludes the alternate", () => {
  ...
  assert.equal(publicGuidePaths().filter(path => path.startsWith("/era/guides/raids/blackwing-lair/")).length, 6);
  for (const encounter of raid.encounters.filter(boss => ![50610, 50611, 50612, 50613, 50614, 50615].includes(boss.id))) assert.equal(bossGuide("era", raid.id, encounter.id), undefined);
...
test("overview coverage permits missing articles but retains identity and complete-raid checks", () => {
  ...
  assert.equal(validateGuides(requireComplete).filter(error => error.includes("missing or duplicate boss coverage")).length, 2);
...
test("BWL Logs resolver uses exact version, zone and encounter IDs", () => {
  for (const [id, slug] of [[50610, "razorgore-the-untamed"], [50611, "vaelastrasz-the-corrupt"], [50612, "broodlord-lashlayer"], [50613, "firemaw"], [50614, "ebonroc"], [50615, "flamegor"]] as const) {
  ...
  assert.equal(bossGuide("era", 2002, 50616), undefined);
