...
const categoryLabel = (category: RaidPrepActivityGroup["category"]) => ({ dungeon: "Dungeon", quest: "Quest", crafted: "Crafted / profession", reputation: "Reputation", "raid-alternative": "Raid alternative", other: "Other" })[category];

function ActivityGroup({ group, version, guildId, raidId, openRunId, coordinatable = true }: { group: RaidPrepActivityGroup; version: string; guildId: string; raidId: string; openRunId?: string; coordinatable?: boolean }) {
  const opportunityLabel = group.category === "raid-alternative" ? `raid alternative${group.opportunityCount === 1 ? "" : "s"}` : `realistic opportunit${group.opportunityCount === 1 ? "y" : "ies"}`;
  return <details className="panel group rounded-2xl p-5">
    <summary className="cursor-pointer list-none rounded-lg focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--accent)]"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-wider text-[var(--muted)]">{categoryLabel(group.category)}</p><h3 className="mt-1 text-xl font-semibold">{group.label}</h3></div><span className="rounded-full border border-[var(--line)] px-3 py-1 text-xs">View players</span></div><p className="mt-3 text-sm"><strong>{group.playerCount}</strong> selected player{group.playerCount === 1 ? "" : "s"} · <strong>{group.opportunityCount}</strong> {opportunityLabel}</p></summary>
    <ul className="mt-5 grid gap-3 border-t border-[var(--line)] pt-4">{group.players.map((player) => <li className="rounded-xl border border-[var(--line)] p-4" key={player.guildCharacterId}><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-semibold">{player.characterName}</p><p className="mt-1 text-xs text-[var(--muted)]">{player.specName} {player.className}{player.groupName ? ` · ${player.groupName}` : ""}{player.raidRole ? ` · ${player.raidRole}` : ""}</p></div><span className="text-sm">{player.opportunityCount} opportunit{player.opportunityCount === 1 ? "y" : "ies"}</span></div>{player.lastRefreshedAt ? <p className="mt-2 text-xs text-[var(--muted)]">Last refreshed {new Date(player.lastRefreshedAt).toLocaleString()}</p> : null}</li>)}</ul>
    {coordinatable ? openRunId ? <Link className="button-primary mt-5 inline-flex" href={`/${version}/guilds/${guildId}/prep/${openRunId}`}>View Prep Run</Link> : <form action={createPrepRunAction} className="mt-5 grid gap-3 border-t border-[var(--line)] pt-5 sm:grid-cols-2"><input type="hidden" name="version" value={version}/><input type="hidden" name="guildId" value={guildId}/><input type="hidden" name="raidId" value={raidId}/><input type="hidden" name="activityKey" value={group.key}/><label className="text-sm">Optional schedule (UTC)<input className="field mt-1 w-full" type="datetime-local" name="scheduledFor"/></label><label className="text-sm">Optional note<input className="field mt-1 w-full" name="note" maxLength={280}/></label><button className="button-primary sm:col-span-2" type="submit">Create Prep Run</button></form> : null}
  </details>;
}
...
export default async function RaidPrepPage({ params, searchParams }: { params: Promise<{ version: string; guildId: string; raidId: string }>; searchParams: Promise<{ prepError?: string }> }) {
  const resolved = await params;
  if (!isContentVersion(resolved.version)) notFound();
  const user = await requireUser();
  let board, runs;
  try { [board, runs] = await Promise.all([getRaidPrepBoard(user.id, resolved.guildId, resolved.raidId), prepRunService.list(user.id, resolved.guildId)]); } catch { notFound(); }
  const query = await searchParams;
  const openRuns = new Map(runs.filter((run) => run.raidId === resolved.raidId && run.status === "open").map((run) => [run.activityKey, run.id]));
  ...
  return <VersionShell version={resolved.version as ContentVersion}><main className="mx-auto max-w-5xl px-5 py-12 lg:px-8">
    ...
    <section aria-labelledby="activities-heading" className="mt-10"><h2 id="activities-heading" className="display text-3xl">Realistic preparation activities</h2><p className="mt-2 text-sm text-[var(--muted)]">Sorted by selected players helped, then opportunity count, then activity name.</p><div className="mt-5 grid gap-4">{board.activityGroups.length ? board.activityGroups.map((group) => <ActivityGroup group={group} version={resolved.version} guildId={resolved.guildId} raidId={resolved.raidId} openRunId={openRuns.get(group.key)} key={group.key}/>) : <EmptyBoard summary={summary}/>}</div></section>
    {board.raidAlternativeGroups.length ? <section aria-labelledby="raid-alternatives-heading" className="mt-10"><h2 id="raid-alternatives-heading" className="display text-3xl">Raid alternatives</h2><p className="mt-2 text-sm text-[var(--muted)]">Shown separately from realistic Pre-Raid preparation. Prep Runs are created from realistic preparation activities above.</p><div className="mt-5 grid gap-4">{board.raidAlternativeGroups.map((group) => <ActivityGroup group={group} version={resolved.version} guildId={resolved.guildId} raidId={resolved.raidId} coordinatable={false} key={group.key}/>)}</div></section> : null}
