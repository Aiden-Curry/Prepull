  ...
  return (
    <article className="panel rounded-2xl p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="eyebrow">{run.raidName} preparation</p>
          <h2 className="mt-2 text-xl font-semibold">{run.activityLabel}</h2>
        </div>
        <span className="rounded-full border border-[var(--line)] px-3 py-1 text-xs capitalize">
          {run.status}
        </span>
      </div>
      ...
      <p className="mt-2 text-sm">
        {run.goingCount} going · {run.maybeCount} maybe
      </p>
      <Link
        className="button-secondary mt-4 inline-flex"
        href={`/${version}/guilds/${run.guildId}/prep/${run.id}`}
      >
        View Prep Run
      </Link>
    </article>
...
export default async function GuildPrepRunsPage({
  params,
  searchParams,
}: {
  ...
  let runs;
  try {
    runs = await prepRunService.list(user.id, resolved.guildId);
  } catch {
    notFound();
  }
  const query = await searchParams;
  const open = runs.filter((run) => run.status === "open"),
    history = runs.filter((run) => run.status !== "open");
  return (
    <VersionShell version={resolved.version as ContentVersion}>
      <main className="mx-auto max-w-4xl px-5 py-12 lg:px-8">
        <nav
          aria-label="Guild preparation navigation"
        ...
        <header className="mt-8">
          <p className="eyebrow">Optional guild coordination</p>
          <h1 className="display mt-2 text-4xl">Prep Runs</h1>
          <p className="mt-3 max-w-2xl text-sm text-[var(--muted)]">
            Guild activities created from raid Prep Board opportunities.
        ...
        <section aria-labelledby="upcoming-heading" className="mt-9">
          <h2 id="upcoming-heading" className="display text-3xl">
            Upcoming Prep Runs
          </h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {open.length ? (
              open.map((run) => (
                <RunCard run={run} version={resolved.version} key={run.id} />
              ))
            ) : (
              <p className="panel rounded-2xl p-5 text-sm text-[var(--muted)] sm:col-span-2">
                No open Prep Runs yet. Raid organizers can create one from a
                realistic Prep Board activity.
              </p>
            )}
          </div>
        </section>
